...
/*	int i = 0;
	...
	// Lets see if makeAction checks isLegalAction even though its
	// supposed to assume its legal.
	while (i < 12){
...
*/	
	...
	// Trades towards GO8's				**NEEDS FUNCTION**
	...
	if ((noMoreCampuses == TRUE && noMoreGO8s == TRUE) ||
	...
	// Trades towards spinoffs			**NEEDS FUNCTION**	
...
static void trader (Game g, action *a, int ID, int type){
	...
	if (getStudents(g, ID, STUDENT_BPS) < costs[STUDENT_BPS - 1]) {
		a->disciplineTo = STUDENT_BPS;
	} else if (getStudents(g, ID, STUDENT_BQN) < costs[STUDENT_BQN - 1]) {
		a->disciplineTo = STUDENT_BQN;
	} else if (getStudents(g, ID, STUDENT_MJ) < costs[STUDENT_MJ - 1]) {
		a->disciplineTo = STUDENT_MJ;
	} else if (getStudents(g, ID, STUDENT_MTV) < costs[STUDENT_MTV - 1]) {
...
static int vertexScore (Game g, path *p, int disc[NUM_REGIONS], int dice[NUM_REGIONS]){
	...
	if (disc[touching[0]] == 0){
		s1 = 0; s2 = dice[touching[1]]; s3 = dice[touching[2]];
	} else if (disc[touching[1]] == 0){
		s1 = dice[touching[0]]; s2 = 0; s3 = dice[touching[2]];
	} else if (disc[touching[0]] == 0){
		s1 = dice[touching[0]]; s2 = dice[touching[1]]; s3 = 0;
	} else {
